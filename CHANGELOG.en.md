...
- **memoQ analyses can be imported, in all four formats memoQ exports them
  ...
  results on screen, the per-file Trados-compatible one and the per-file one
  with all the information. Before, its CSV could be picked but fell into a
  single column — memoQ never separates with commas — and its HTML was turned
  away with "this is not a Trados report". Rows arrive already assigned to their category — 101% and
  pre-translated ones as context, repetitions, 100%, each band and the ones
  with no match — and "Fragments" is shown unassigned, like "Locked" in
  Trados: whether that is billed is each client's call. If the file holds
  several analyses one after another, the first one is imported and the dialog
  says so; if it holds several files, they are added up, without counting the
  totals row twice. Checked with the same analysis exported in all four
  formats: all four give 249 words and the same breakdown, and from there
  €18.10 at €0.09/word, both on screen and in the quote PDF.

- **Wordfast Pro analyses can be imported too, in the four formats a
  translator uses.** "Import CAT analysis" reads what its "Export report"
  produces: its own CSV, the Trados-compatible CSV, the HTML and the XLS. Rows
  arrive already assigned — context, repetitions, 100%, each band and the ones
  with no match — and "Ignored" segments are shown unassigned, like "Locked" in
  Trados. If the report holds several files, they are added up, and its totals
  table is not counted on top of them. Wordfast writes five more formats, meant
  to feed other programs: some do not even carry the repetitions or the new
  words, so they are not recognised and the file goes to the generic importer,
  where columns are picked by hand. Checked with reports generated by Wordfast
  itself: all four formats give the same 311 words and the same breakdown, and
  from there €22.09 at €0.09/word, both on screen and in the quote PDF.
...
- **A newly created project no longer looks quoted.** Until now it started with
  ...
  "Quoted" itself, dashed and unfilled, and the hint reads "Click on Quoted to
  advance": the quote can be edited and sent as many times as needed, and the
  click, once the client accepts it, moves the project to "In progress".
- **The quote has its own card**, like the job and the invoice, instead of
  sitting inside the status bar. Its buttons read "Edit", "Download" and
  "Send" (or "Resend", once it has gone out), without repeating "quote" on all
  three. Once the project moves on, the card keeps the sending date and the
  download. The Invoice card follows suit: "Edit", "Download" and "Send".
- **Dates spell out the month**: "Sep 26, 2026" in English and "26 sept 2026"
  in Spanish, each language's convention (the CLDR medium format that browsers
  apply). They used to be numeric and switched order with the
  language (9/26/2026 vs 26/9/2026), so 3/5 could be March or May. This
  applies across the whole app — detail panels, lists, metrics, history,
  notices — and to the invoice and quote PDFs, which stay in English: "Sep 26,
  2026" with the interface in English, "26 Sep 2026" otherwise.
- **A custom calendar for picking dates**, in all seven date fields. The
  Windows/Chromium one showed the date in the format of the Windows language,
  not the app's, and its mouse wheel scrolled weeks without changing the month
  in the header. Now the wheel moves between months, days from the previous
  and next month are greyed out, and there is a "Today" button (plus "Clear"
  where the date is optional). Keyboard: arrows to move, Page Up/Page Down to
  change month, Enter to pick, Esc to close.
- **"Send deliverables" is locked until the quote is accepted**, like "Create
  invoice" before delivery, and says what is missing: "Mark Quoted before
...
- **A semicolon-separated CSV is no longer read as a single column.** Excel in
  Spanish and French exports "CSV (semicolon delimited)", and memoQ's
  statistics are only exported with semicolons or tabs. The app read
  everything as if it were commas, so the whole file fell into one column:
  importing contacts showed one very long header and nothing else, and
  importing an analysis recognised no category at all. The separator is now
  decided by reading the file itself, not its extension, and a comma inside
  "Dupont, Marie" is still part of the name.
